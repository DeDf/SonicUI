...
LONG (__cdecl * _DbgPrint)(LPCSTR Format, ... );
					 ...
					 )
{
	if(ul_reason_for_call == DLL_PROCESS_ATTACH)
	{
        ...
        *(FARPROC *)&_DbgPrint = GetProcAddress(GetModuleHandle("ntdll.dll"), "DbgPrint");
	}
    return TRUE;
}
...
{
    ISonicImage *p = new CSonicImage;
	m_CreatedList.push_back(p);
	return p;
}
...
{
    ISonicString *p = new CSonicString;
	m_CreatedList.push_back(p);
	return p;
}
